...
FRANK_WOLFE_STEPSIZE_PRECISION = 1e-6
...
class Network:
   ...
   def alphacj(self, allornothing, oldtargetflow,eps=0.01):
      num=0
      den=0
      for i in self.link:
         fac1=oldtargetflow[i]-self.link[i].flow
         tij= self.link[i].freeFlowTime*self.link[i].alpha*self.link[i].beta* pow((fac1/self.link[i].capacity),self.link[i].beta-1)/(self.link[i].capacity)
         fac2= allornothing[i]-self.link[i].flow
         fac3= allornothing[i]-oldtargetflow[i]
         num+= tij*fac2
         den+=tij*fac3
      if den==0:
         return 0
   ...
   def userEquilibrium(self, stepSizeRule = 'MSA',
      ...
      stepSize=self.Newtons(targetflows)
      ...
      while iteration < maxIterations:
         ...
         AON=self.allOrNothing()
         alpha= self.alphacj(AON, targetflows)
         
         for i in self.link:
            targetflows[i] = (1-alpha)*AON[i]+alpha*targetflows[i]
         if stepSizeRule == 'FW':
            stepSize = self.Newtons(targetflows)
